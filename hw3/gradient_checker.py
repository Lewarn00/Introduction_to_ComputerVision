...
# initialize avg pool
student_avgpool = student.AvgPool2d(kernel_size=2, stride=2)
ref_avgpool = nn.AvgPool2d(kernel_size=2, stride=2)
# align input data
avgpool_data_pt = Variable(torch.rand(1, 3, 32, 32), requires_grad = True)
avgpool_data_numpy = avgpool_data_pt.data.numpy()
# check output
ref_out = ref_avgpool(avgpool_data_pt)
student_out = student_avgpool.forward(avgpool_data_numpy)
print_shape_match(ref_out.data.numpy(), student_out)
print_res_match(ref_out.data.numpy(), student_out)
# check gradients
grad_out = student_avgpool.backward(np.ones_like(student_out))
ref_out.backward(torch.ones_like(ref_out))
print_res_match(avgpool_data_pt.grad.data.numpy(), grad_out)
print('\n')

print('Testing ELU:')
# initialize ELU
student_elu = student.ELU()
ref_elu = nn.ELU()
# align input data
elu_data_pt = Variable(torch.rand(1, 3, 32, 32), requires_grad = True)
elu_data_numpy = elu_data_pt.data.numpy()
# check output
ref_out = ref_elu(elu_data_pt)
student_out = student_elu.forward(elu_data_numpy)
print_shape_match(ref_out.data.numpy(), student_out)
print_res_match(ref_out.data.numpy(), student_out)
# check gradients
grad_out = student_elu.backward(np.ones_like(student_out))
ref_out.backward(torch.ones_like(ref_out))
print_res_match(elu_data_pt.grad.data.numpy(), grad_out)
print('\n')

print('Testing BatchNorm2d:')
# initialize BN
student_bn = student.BatchNorm2d(100)
ref_bn = nn.BatchNorm2d(100)
# align input data
bn_data_pt = Variable(torch.rand(20, 100, 10, 10), requires_grad = True)
